...
private const val FoodEntryCounterKey = "me.mpardo.dailycaloriecoutner.FoodEntryCounterJsonDb"
...
class SharedPreferenceFoodEntryCounterRepository : FoodEntryCounterRepository, KoinComponent {
    ...
    data class FoodEntryCounterPref(
        ...
        fun into(foodEntryRepository: FoodEntryRepository) =
            FoodEntryCounter(foodEntryRepository.get(foodEntryId)!!, counter)
    ...
    override fun get(key: FoodEntry): FoodEntryCounter? =
        allPref.find { it.foodEntryId == key.id }?.into(foodEntryRepository)
    ...
    override fun update(e: FoodEntryCounter) {
        json =
            Json.encodeToString(allPref.map { if (it.foodEntryId == e.food.id) e.into() else it })
    }
}